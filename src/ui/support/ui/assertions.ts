...
export type ScreenAssertions = Readonly<{
  ...
  expectListItemInList: (listLocator: Locator, itemText: string | RegExp) => Promise<void>;
  expectAllListItemsInList: (listLocator: Locator, expectedItems: string[]) => Promise<void>;
  expectRowsToContainText: (container: Locator, expectedRows: string[]) => Promise<void>;
}>;
...
export function createScreenAssertions(page: Page): ScreenAssertions {
  return {
    ...
    async expectListItemInList(listLocator: Locator, itemText: string | RegExp) {
      const listItem = listLocator.locator('li').filter({ hasText: itemText });
      await expect(listItem).toBeVisible();
    },

    async expectAllListItemsInList(istLocator: Locator, expectedItems: string[]) {
      for (const text of expectedItems) {
        await softExpect(istLocator.locator('li').filter({ hasText: text })).toBeVisible();
      }
    },

    async expectRowsToContainText(container: Locator, expectedRows: string[]) {
      for (const text of expectedRows) {
        const row = container.locator('> *').filter({ hasText: text });
