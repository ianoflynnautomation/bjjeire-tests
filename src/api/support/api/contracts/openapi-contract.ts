...
const OpenApiSchemaShape = z.custom<OpenApiSchema>(isOpenApiSchemaLike);

const OpenApiMediaTypeSchema = z
  .object({
    schema: OpenApiSchemaShape.optional(),
  })
  .loose();

const OpenApiContentSchema = z.record(z.string(), OpenApiMediaTypeSchema);

const OpenApiOperationSchema = z
  .object({
    requestBody: z
      .object({
        content: OpenApiContentSchema.optional(),
      })
      .loose()
      .optional(),
    responses: z
...
const OpenApiDocumentSchema = z
  .object({
    ...
    components: z
      .object({
        schemas: z.record(z.string(), OpenApiSchemaShape).optional(),
      })
      .loose()
      .optional(),
  })
...
async function readOpenApiContract(apiClient: APIRequestContext): Promise<OpenApiDocument> {
  ...
  if (artifactPath && existsSync(artifactPath)) {
    return parseOpenApiDocument(parseJsonText(readFileSync(artifactPath, 'utf8'), artifactPath), artifactPath);
  }
...
function parseJsonText(raw: string, source: string): unknown {
  try {
    return JSON.parse(raw);
  } catch (error: unknown) {
    const message = error instanceof Error ? error.message : String(error);
    throw new Error(`OpenAPI contract from ${source} is not valid JSON: ${message}`);
  }
}
...
function isOpenApiSchemaLike(value: unknown): value is OpenApiSchema {
